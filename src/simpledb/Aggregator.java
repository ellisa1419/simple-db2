...
public interface Aggregator {
	...
	class MinAggregateFunction implements AggregateFunction {
		...
		public void merge(Field value) {
			...
			if (value.compare(Predicate.Op.LESS_THAN, min)){
				min=value;
			}
	...
	class MaxAggregateFunction implements AggregateFunction {
		...
		public void merge(Field value) {
			...
			if (value.compare(Predicate.Op.GREATER_THAN, max)){
				max=value;
			}
		}
	...
	class IntSumAggregateFunction implements AggregateFunction {
		...
		Field sum= new IntField(0);
		

		public void merge(Field value) {
			int	total=Integer.parseInt(sum.toString())+Integer.parseInt(value.toString());
			sum= new IntField(total);
